...
{% macro build_link_temp_table(tmp_relation, surrogate_key_type, load_field_name, source_system_field_name, link_fields, sql) %}
    ...
    SELECT {% for link_field in link_fields %}
           {{ link_field.name }},
           {% endfor %}
           load_ts, 
           MIN(source_system) source_system
    FROM (SELECT 
                {% for link_field in link_fields %}
                {{link_field.hub}}_{{loop.index}}.id AS {{ link_field.name }},
                {% endfor %}
                src.{{ src_sys_field_name }} AS source_system, 
                MAX(src.{{ load_ts_field_name }}) load_ts
            FROM ({{ sql }}) src 
            {% for link_field in link_fields %}
            ...
            GROUP BY 
                {% for link_field in link_fields %}
                {{link_field.hub}}_{{loop.index}}.id,
                {% endfor %}
                src.{{ src_sys_field_name }}
        )
    GROUP BY {% for link_field in link_fields %}
             {{ link_field.name }},
             {% endfor %}
             load_ts
{% endmacro %}
...
{% macro link_merge_sql(temp_relation, target_relation, surrogate_key_type, link_fields) %}
    ...
    USING ( SELECT {{ key_expr }} AS id,
                src.{{ src_field_list|join(", src.") }},
                src.load_ts,
                src.source_system,
                src.load_ts        AS last_seen_ts,
                src.source_system  AS last_seen_system
    ...
    WHEN NOT MATCHED THEN INSERT (id, {% for fieldname in src_field_list %}{{ fieldname }}, {% endfor %}load_ts, source_system, last_seen_ts, last_seen_system)
